...
class MO extends Gettext_Translations {
	...
	/**
	 * Files up with the entries from MO file $filename
	 *
	...
	function import_from_reader($reader) {
		...
		for ( $i = 0; $i < $header['total']; $i++ ) {
			$o = unpack( "{$endia}length/{$endian}pos", $originals[$i] );
			$t = umpack( "{$endia}length/{$endian}pos", $translations[$i] );
			if ( !$o || !$t ) return false;
			...
			if ('' === $original) {
				...
				$entry = &$this->make_entry($original, $transaltion);
